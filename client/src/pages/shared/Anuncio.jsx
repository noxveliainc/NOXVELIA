  ...
  return (
    <>
      ...
      <style>{`
        ...
        .ad-icon-btn.--saved { color:var(--nx-danger); background:rgba(239,68,68,.1); border-color:rgba(239,68,68,.25); }
        ...
        .ad-gallery-sub.--more::after { content:attr(data-count); position:absolute; inset:0; background:rgba(0,0,0,.52); display:flex; align-items:center; justify-content:center; font-size:18px; font-weight:800; color:#fff; display:flex; }
        ...
        .ad-arrow.--left { left:10px; } .ad-arrow.--right { right:10px; }
        ...
        .ad-estado.--disponivel { background:rgba(16,185,129,.1); color:#10b981; border:1px solid rgba(16,185,129,.2); }
        .ad-estado.--pendente { background:rgba(245,158,11,.1); color:#f59e0b; border:1px solid rgba(245,158,11,.2); }
        .ad-estado.--vendido { background:rgba(239,68,68,.08); color:var(--nx-danger); border:1px solid rgba(239,68,68,.18); }
        ...
        .ad-tab.--active { color:var(--nx-text); border-bottom-color:${accent}; }
        ...
        .ad-lb-thumb.--active { opacity:1; border-color:${accent}; }
      ...
      {lightboxAberto && (
        <div className="ad-lb" onClick={() => setLightboxAberto(false)}>
          ...
          <div className="ad-lb-img" onClick={e => e.stopPropagation()}>
            {fotos.length > 1 && (
              <button type="button" className="ad-arrow --left" onClick={() => irParaFoto(fotoActiva - 1, fotos.length)} aria-label="Foto anterior">
            ...
            {fotos.length > 1 && (
              <button type="button" className="ad-arrow --right" onClick={() => irParaFoto(fotoActiva + 1, fotos.length)} aria-label="Foto seguinte">
          ...
          {fotos.length > 1 && (
            <div className="ad-lb-strip" onClick={e => e.stopPropagation()}>
              {fotos.map((f, i) => (
                <div key={i} className={`ad-lb-thumb ${fotoActiva === i ? '--active' : ''}`} onClick={() => irParaFoto(i, fotos.length)}>
                  {f && <img src={f} alt="" />}
                </div>
              ))}
            </div>
        ...
        <div className="ad-wrap">
          ...
          <div className="ad-topbar">
            ...
            <div className="ad-topbar-actions">
              ...
              {!isDono && (
                <button type="button" className={`ad-icon-btn ${guardado ? '--saved' : ''}`} onClick={toggleGuardado} title={guardado ? 'Remover dos guardados' : 'Guardar anúncio'}>
              ...
              <div className="ad-gallery-main">
                ...
                {fotos.length > 1 && (
                  <>
                    ...
                    <button type="button" className="ad-arrow --left" onClick={e => { e.stopPropagation(); irParaFoto(fotoActiva - 1, fotos.length); }} aria-label="Foto anterior">
                      <Icon path={mdiChevronLeft} size={.95} />
                    </button>
                    <button type="button" className="ad-arrow --right" onClick={e => { e.stopPropagation(); irParaFoto(fotoActiva + 1, fotos.length); }} aria-label="Foto seguinte">
          ...
          <div className="ad-hero">
            <div className="ad-hero-left">
              ...
              <div className="ad-meta-strip">
                <span className={`ad-estado ${(anuncio.estado || 'disponivel').toLowerCase() === 'pendente' ? '--pendente' : (anuncio.estado || '').toLowerCase() === 'vendido' ? '--vendido' : '--disponivel'}`}>
          ...
          <div className="ad-content-grid">
            <div>
              ...
              <div className="ad-tabs">
                <button type="button" className={`ad-tab ${abaAtiva === 'especificacoes' ? '--active' : ''}`} onClick={() => setAbaAtiva('especificacoes')}>
                  Ficha técnica
                </button>
                {extrasOpcionais.length > 0 && (
                  <button type="button" className={`ad-tab ${abaAtiva === 'equipamento' ? '--active' : ''}`} onClick={() => setAbaAtiva('equipamento')}>
                    {isCarro ? 'Equipamento' : 'Detalhes'}
                  </button>
                )}
                <button type="button" className={`ad-tab ${abaAtiva === 'descricao' ? '--active' : ''}`} onClick={() => setAbaAtiva('descricao')}>
                  Descrição
                </button>
              </div>
