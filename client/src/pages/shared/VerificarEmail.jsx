...
export default function VerificarEmail() {
  ...
  const [estado, setEstado] = useState('a_verificar'); // 'a_verificar' | 'sucesso' | 'erro'
  const [mensagem, setMensagem] = useState('');

  useEffect(() => {
    const confirmar = async () => {
      try {
        const resposta = await api.get(`/auth/verify-email/${token}`);
        setMensagem(resposta.data?.mensagem || 'Email verificado com sucesso!');
        setEstado('sucesso');
      } catch (err) {
        setMensagem(err.response?.data?.erro || 'Não foi possível verificar o teu email.');
        setEstado('erro');
      }
    };
      ...
      <div className="auth-root">
        <div className="auth-card">
          ...
          <div style={{ marginBottom: '24px' }}>
            <img src="/logo-noxvelia.png" alt="NOXVELIA" style={{ height: '32px', width: 'auto', objectFit: 'contain', margin: '0 auto' }} />
          </div>
          ...
          {estado === 'a_verificar' && (
            <>
              ...
              <h1 className="auth-title">A verificar a tua conta...</h1>
              <p className="auth-subtitle">Aguarda um momento, estamos a confirmar o teu email.</p>
            </>
          )}

          {estado === 'sucesso' && (
            <>
              <div className="verify-icon sucesso">
                <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  <polyline points="20 6 9 17 4 12" />
                </svg>
              </div>
              <h1 className="auth-title">Conta verificada!</h1>
              <p className="auth-subtitle">{mensagem} Já podes iniciar sessão na plataforma.</p>
              <Link to="/login" className="auth-btn">Iniciar Sessão</Link>
            </>
          )}

          {estado === 'erro' && (
            <>
              <div className="verify-icon erro">
                <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  <line x1="18" y1="6" x2="6" y2="18" />
                  <line x1="6" y1="6" x2="18" y2="18" />
                </svg>
              </div>
              <h1 className="auth-title">Não foi possível verificar</h1>
              <p className="auth-subtitle">{mensagem}</p>
              <Link to="/login" className="auth-btn outline">Voltar ao Login</Link>
            </>
          )}
