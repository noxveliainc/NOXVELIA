...
export default function Login() {
  ...
  const [mostrarPassword, setMostrarPassword] = useState(false); // 🌟 NOVO ESTADO
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        const destinoOrigem = location.state?.from || fallbackUniverso || '/';
        
        navigate(destinoOrigem, { replace: true });
      }, 1000);
    } catch (err) {
      console.error(err);
  ...
  return (
    <>
      <style>{`
        ...
        /* 🌟 NOVO CSS PARA O ENVOLVÓRIO DA PASSWORD */
        .auth-input-wrapper {
          position: relative;
          display: flex;
          align-items: center;
      ...
      <div className="auth-root">
        <div className="auth-card">
          <Link to={location.state?.from || "/"} className="auth-back">← Voltar</Link>
