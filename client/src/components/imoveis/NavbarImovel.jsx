  ...
  return (
    <>
      <style>{`
        ...
        .nim-mobile-drawer { position: fixed; inset: 0; z-index: 9999; pointer-events: none; }
        ...
        .nim-drawer-content { position: absolute; top: 0; right: 0; width: 290px; height: 100vh; background: #ffffff; box-shadow: -10px 0 40px rgba(0,0,0,0.15); transform: translateX(100%); transition: transform 0.3s cubic-bezier(0.16, 1, 0.3, 1); padding: 24px; box-sizing: border-box; display: flex; flex-direction: column; pointer-events: auto; }
        
        .nim-mobile-drawer.active { pointer-events: auto; }
        ...
        .nim-drawer-menu { display: flex; flex-direction: column; gap: 8px; flex: 1; }
        ...
        <div className={`nim-mobile-drawer ${menuMobileAberto ? 'active' : ''}`}>
          ...
          <div className="nim-drawer-content">
            ...
            <div className="nim-drawer-menu">
              {signed ? (
                <>
                  <Link to="/publicar" className="nim-drawer-link publish" onClick={() => setMenuMobileAberto(false)}>+ Criar Anúncio</Link>
                  <Link to="/perfil" className="nim-drawer-link" onClick={() => setMenuMobileAberto(false)}>👤 O Meu Perfil {isPremium && <span className="nim-ud-pro">PRO</span>}</Link>
                  <button type="button" className="nim-drawer-link" onClick={handleIrParaHome}>🏠 Início (Estate)</button>
                  <button type="button" className="nim-drawer-link" onClick={handlePremium}>💼 Plano Profissional {isPremium && '✓'}</button>
                  <Link to="/favoritos" className="nim-drawer-link" onClick={() => setMenuMobileAberto(false)}>❤️ Favoritos</Link>
                  <button type="button" className="nim-drawer-link" onClick={() => { setMenuMobileAberto(false); setMostrarModalPassword(true); }}>🔐 Alterar Password</button>
                  <button type="button" className="nim-drawer-link logout-btn" onClick={() => { setMenuMobileAberto(false); logout(); }}>🚪 Terminar Sessão</button>
                </>
              ) : (
                <>
