  ...
  return (
    <>
      ...
      @import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700&family=Plus+Jakarta+Sans:wght@800&display=swap');
        ...
        .nim-btn-sponsor:hover { border-color: #d9c49c; background: rgba(217, 196, 156, 0.28); color: #071326; }
        ...
        .nim-user-dropdown { background: #fffaf0 !important; border-color: rgba(217, 196, 156, .38) !important; }
        .nim-ud-item { color: #102f50 !important; }
        .nim-ud-item:hover { color: #071326 !important; background: rgba(16, 47, 80, .08) !important; }
        .nim-ud-divider { background: rgba(16, 47, 80, .12) !important; }
        .nim-ud-item.logout:hover { color: #b42318 !important; background: #fff3f1 !important; }
        .nim-pro-badge,
      ...
      <nav className={`nim-root${scrolled ? ' scrolled' : ''}`}>
        ...
        <div className="nim-actions">
          ...
          <Link to="/patrocinios" className="nim-btn-menu nim-btn-sponsor">
          ...
          {signed ? (
            <>
              ...
              <button type="button" onClick={handlePRO} className={`nim-btn-premium${isPRO ? ' active' : ''}`}><svg viewBox="0 0 24 24"><rect x="2" y="7" width="20" height="14" rx="2" ry="2" /><path d="M16 7V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v2" /><line x1="12" y1="12" x2="12" y2="16" /><line x1="10" y1="14" x2="14" y2="14" /></svg></button>
              ...
              <div ref={userMenuRef} style={{ position: 'relative' }}>
                <button className={`nim-user-trigger ${userMenuAberto ? 'active' : ''}`} onClick={() => setUserMenuAberto(!userMenuAberto)}>
                  ...
                  {primeiroNome && <span className="nim-username">{primeiroNome} {isPRO && <span className="nim-pro-badge">PRO</span>}</span>}
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M6 9l6 6 6-6"/></svg>
                </button>
                {userMenuAberto && (
                  <div className="nim-user-dropdown" onClick={(e) => e.stopPropagation()}>
                    <Link to="/perfil" onClick={() => setUserMenuAberto(false)} className="nim-ud-item"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"></path><circle cx="12" cy="7" r="4"></circle></svg>O Meu Perfil {isPRO && <span className="nim-ud-pro">PRO</span>}</Link>
                    {isAdmin && (
        ...
        <div id="nim-mobile-drawer" className={`nim-mobile-drawer ${menuMobileAberto ? 'active' : ''}`} role="dialog" aria-modal={menuMobileAberto ? 'true' : undefined} aria-hidden={!menuMobileAberto}>
          ...
          <div className="nim-drawer-content">
            <div className="nim-drawer-header">
              {signed ? (
                <div className="nim-drawer-user">
                  <div className="nim-avatar">{avatarImg ? <img src={avatarImg} alt="Perfil" /> : <span className="nim-avatar-initial">{inicial}</span>}</div>
                  <span className="nim-drawer-title">Olá, {primeiroNome}!</span>
                </div>
              ) : (
                <span className="nim-drawer-title">Menu</span>
              )}
            ...
            <div className="nim-drawer-menu">
              ...
              <Link to="/enviar-stock" className="nim-drawer-link" onClick={() => setMenuMobileAberto(false)}>
                <svg viewBox="0 0 24 24"><path d="M4 5h16v14H4z" /><path d="M8 9h8M8 13h5" /></svg>
                Enviar stock
              </Link>
              <Link to="/patrocinios" className="nim-drawer-link sponsor" onClick={() => setMenuMobileAberto(false)}>
                <svg viewBox="0 0 24 24"><path d="M4 13V7a2 2 0 0 1 2-2h7l7 4v6l-7 4H6a2 2 0 0 1-2-2v-4z" /><path d="M13 5v14" /><path d="M7 19v2" /></svg>
              ...
              {signed ? (
                <>
                  ...
                  <Link to="/perfil" className="nim-drawer-link" onClick={() => setMenuMobileAberto(false)}>
                    ...
                    O Meu Perfil {isPRO && <span className="nim-ud-pro">PRO</span>}
                  </Link>
                  <button type="button" className="nim-drawer-link" onClick={handlePRO}>
                    ...
                    Plano PRO {isPRO && <span className="nim-ud-pro">PRO</span>}
                  </button>
