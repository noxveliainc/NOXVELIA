...
export default function NavbarImovel() {
  ...
  const alterarPassword = async (e) => {
    ...
    try {
      ...
      alert(err.response?.data?.erro || 'Erro ao alterar a palavra-passe.');
    } finally {
    ...
    <>
      <style>{`
        ...
        @media (max-width: 640px) {
          ...
          .nim-btn-publish { padding: 8px 14px; margin-right: 4px; font-size: 12px; }
          .nim-divider { margin: 0 4px; }
          .nim-username { display: none; }
          .nim-user-trigger { padding: 0; border: none; }
        }
      `}</style>
      ...
      {mostrarModalPassword && (
        <div className="nav-modal-overlay">
          <div className="nav-modal-card">
            ...
            <h2 className="nav-modal-title">🔐 Alterar Palavra-passe</h2>
      ...
      <nav className={`nim-root${scrolled ? ' scrolled' : ''}`}>
        ...
        <div
          ref={logoRef}
          className={`nim-logo-wrapper ${dropdownAberto ? 'active' : ''}`}
        ...
        >
          <div className="nim-logo">
            ...
            <div className="nim-logo-brand-text">
              NOXVELIA{' '}
              <span style={{ color: location.pathname.includes('carros') ? '#2ac1b4' : '#3ecf8e' }}>
                {location.pathname.includes('carros') ? 'Drive' : 'Estate'}
              </span>
            </div>
          </div>
          ...
          {dropdownAberto && (
            <div className="nim-switcher-dropdown" onClick={e => e.stopPropagation()}>
              <Link to="/imoveis" className="nim-switcher-item current">NOXVELIA Estate</Link>
              <Link to="/carros" className="nim-switcher-item">NOXVELIA Drive</Link>
            </div>
          )}
        </div>
        ...
        <div className="nim-actions">
          {signed ? (
            <>
              ...
              {/* Casa */}
              <button type="button" onClick={handleIrParaHome} className="nim-icon-btn" title="Início — NOXVELIA Estate">
              ...
              <button
                ...
                className={`nim-btn-premium${isPremium ? ' active' : ''}`}
                title={isPremium ? 'Plano Profissional Ativo' : 'Aderir ao Plano Profissional'}
              >
                <svg viewBox="0 0 24 24">
              ...
              {/* Favoritos */}
              <Link to="/favoritos" state={{ from: location.pathname }} className="nim-icon-btn" title="Favoritos">
                <svg viewBox="0 0 24 24">
              ...
              <div ref={sinoRef} style={{ position: 'relative' }}>
                <button type="button" className="nim-icon-btn" onClick={() => setSinoAberto(!sinoAberto)} title="Notificações">
                  <svg viewBox="0 0 24 24">
                    ...
                    <div className="nim-sino-body">
                      {notificacoes.length > 0
                        ? notificacoes.map(n => (
                          <div
                            key={n._id}
                            className={`nim-sino-item ${!n.lida ? 'unread' : ''}`}
                            onClick={() => handleLerNotificacao(n)}
                          >
              ...
              <div ref={userMenuRef} style={{ position: 'relative' }}>
                <button
                  className={`nim-user-trigger ${userMenuAberto ? 'active' : ''}`}
                  onClick={() => setUserMenuAberto(!userMenuAberto)}
                >
                  <div className="nim-avatar">
                    {avatarImg
                      ? <img src={avatarImg} alt="Perfil" />
                      : <span className="nim-avatar-initial">{inicial}</span>
                    }
                  </div>
                  {primeiroNome && (
                    <span className="nim-username">
                      {primeiroNome}
                      {/* 🌟 BADGE PRO */}
                      {isPremium && <span className="nim-pro-badge">PRO</span>}
                    </span>
                  )}
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{ color: '#94a3b8' }}>
                    <path d="M6 9l6 6 6-6" />
                  </svg>
                </button>
                ...
                {userMenuAberto && (
                  <div className="nim-user-dropdown" onClick={e => e.stopPropagation()}>
                    <Link to="/perfil" onClick={() => setUserMenuAberto(false)} className="nim-ud-item">
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                        <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" /><circle cx="12" cy="7" r="4" />
                      </svg>
                      O Meu Perfil
                      {/* 🌟 BADGE PRO no dropdown */}
                      {isPremium && <span className="nim-ud-pro">PRO</span>}
                    </Link>
                    <button
                      onClick={() => { setUserMenuAberto(false); setMostrarModalPassword(true); }}
                      className="nim-ud-item"
                    >
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
